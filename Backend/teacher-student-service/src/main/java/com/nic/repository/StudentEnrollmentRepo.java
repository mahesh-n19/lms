...
public interface StudentEnrollmentRepo extends JpaRepository<StudentEnrollment, Long> {
	...
	// classroom count by student id for student dashboard
	@Query(value = "SELECT COUNT(*) FROM student_enrollment WHERE student_id = :studentId", nativeQuery = true)
	int getCountClassroomByStudentID(@Param("studentId") int studentId);
