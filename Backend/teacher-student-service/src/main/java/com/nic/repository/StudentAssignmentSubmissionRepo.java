...
public interface StudentAssignmentSubmissionRepo extends JpaRepository<StudentAssignmentSubmission, Long> {
	...
	@Query("select new com.nic.dto.SubmittedAssignmentStudentDetailsDto(S.submissionId, U.name, S.submissionDate) FROM StudentAssignmentSubmission S INNER JOIN User U on S.studentId = U.userId where S.assignmentId=?1")
